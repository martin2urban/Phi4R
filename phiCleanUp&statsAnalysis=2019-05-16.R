...
##  followed by stats analysis   2019-05-16
##
#--Environment----
...
#library(xlsx) and library("rJava")  is needed for wrtining xlsx files in R;  currently ("rJava") is not compatible with my set up!
library(writexl)
library(here)
library(janitor)  #needed for 'excel_Numeric_to_date'
#--MAIN---------------------------------------------------------
#  global variables
directory="Phi47"
...
#   READ FROM EXCEL
####################################################################
phi=read_excel(here("Phi47/phi47.xlsx"), sheet= "PHIbase_all")
#test spreadsheet beneath if needed
#phi=read_excel(here("Phi47/phiMuTest3.xlsx"), sheet= "PHIbase_all")
phi1=phi  %>% as.tbl #read dataset and trim empty Excel columns and header off

# now clear col 1 and 2: extra long comments
phi1[,1:2]=""

# set date format for excel
phi$"Curation date"= excel_numeric_to_date(as.numeric(phi$"Curation date"))
...
#phi=phi[1:85]
#remove additonal columns from Alayne not displayed in PHI4
excludeColsPhi44=c(28,49,52,53)  #cols in spreadsheet but not in MC parser
phi2=phi1 %>% select(-excludeColsPhi44)
####################################################################
###   reformat columns and clean up spreadsheet
####################################################################

# remover all leading+trailing+duplicated white space between words
trim <- function (x) gsub("^\\s+|\\s+$", "", x)
replaceWhite <- function (x) gsub("\\s+", " ", x)
phi3=phi2 %>% mutate_all(funs(trim)) %>%mutate_all(funs(replaceWhite))
#%>% mutate_each(funs(tolower))
capitalFirstWord <- function(x) gsub("(?<=\\b)(^[a-z])", "\\U\\1", tolower(x), perl=TRUE)
colsForCapitals=c(20,23,26,56) #20=Pathogen_species, 23=Disease_name, 26=Experimental_Host_species, 56=Experimental_evidence
...
####################################################################
####################################################################
##  ADD 2nd Line PHIbase MC header
####################################################################
phiX=phi2[,1:79]
#mcHeader=fread(here("Phi47/phiHeadersPhi47.csv"),select=c(1),colClasses = list(character=4))
phiX$"Curation date"=""   # for unknown reason the data makes problems during R bind, perhaps because the Mc header has no date!
mcHeader1=read_excel(here("Phi47/phiHeadersPhi47.xlsx"),  range="sheet1!A2:A80", col_names=FALSE, na="",  trim_ws=TRUE, skip=0)
mcHeader2=unlist(mcHeader1)
newdata= rbind(mcHeader2,phiX) 
####################################################################
#   WRITE TO EXCEL
####################################################################
#  NOTE:  TAXONIDS   entered as numbers initially into Excel are written from r as TEXT! note: Does not need to be changed.
fileLocation=paste(directory,"/","phi47ForParse",Sys.Date(),".xlsx",sep="")
write_xlsx(phi2, here(fileLocation))
####################################################################

### continue with STATISTICS ANALYSIS


###   BENEATH IS FURTHER ANALYSIS ########
